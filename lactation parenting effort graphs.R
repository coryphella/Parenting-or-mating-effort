...
pdf("Posterior_Mean_DSIs_Male_Father_Status_Lactating_Females.pdf", height=7,width=7)
...
mtext(side=1,line=1.25,cex=1.4,text="dyadic sociality index (DSI): lactation")
#points below
...
#Plot rank
pdf("Rank_Effect_All Males_Lactating_Females_Shade89CI.pdf", height=7,width=7)
...
pdf("Rank_Effect_All Males_Lactating_Females_Indiv_Preds.pdf", height=7,width=7)
par(mar=c(3.5,3.5,0.5,1.5))
...
#plot rank
pdf("Rank_Interaction_Triptych_Males_Lactating_Females.pdf", height=3.25,width=8)
...
pdf("Rank_Interaction_Triptych_Males_Lactating_Females_Indiv_Preds.pdf", height=3.25,width=8)
...
##########################################################################
#############################################################################
#plot Bernoulli and gamma component separately
...
par(mfrow=c(3,1) , mar=c(c(3.25,0,0,0)) , oma=c(0.5,.5,.5,0.5))
dens(pred_nodad_z , xlab=NA , ylab=NA , col="white"  , xaxt='n' ,  yaxt='n', zero.line = FALSE,xlim=c(0,1),ylim=c(0,45))
axis(1, tck=-0.015,cex=1,labels=NA,at=seq(0,1,by=0.1))
axis(1, cex.axis=0.8,at= c(0,0.5,1),labels=c(0,0.5,1),line=0.1,col=NA)
...
mtext(side=1,line=2,"Probability of dyadic interaction (DSI > 0)",cex=0.8)

legend("topleft", legend = c("sires of current infant","sires of next infant","other males"),
       col=c(col.alpha("blue", 0.5) ,col.alpha("red", 0.5), col.alpha("orange", 0.5) ) , pch=c(15,17,16),
       pt.cex=1.5 , bty="n",y.intersp=1.2,x.intersp=2, lty=c(0,0,0) , lw=1)
...
axis(1, tck=-0.015,cex=1,labels=NA,at=seq(0,10,by=1))
axis(1, cex.axis=0.8,at= c(0,5,10),labels=c(0,5,10),line=0.1,col=NA)
...
mtext(side=1,line=2,"DSI conditional on dyadic interaction",cex=0.8)
...
mtext(side=1,line=2,cex=0.8,text="Dyadic sociality index (DSI)")
