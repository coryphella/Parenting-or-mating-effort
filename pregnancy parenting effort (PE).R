...
d$PHG <- ifelse(d$preg_residence=="PHG" , 1 , 0 )
...
save(d, p_preg0, p_preg1, p_preg2, p_preg3, p_preg4 , file="parent_eff_preg_jan30.rdata")

#########relevant coefficient and model checking codes and plots##########
plot(p_preg0)
par(mfrow = c(1, 1))
output_preg0=precis(p_preg0, depth=2 , digits=2)@output

plot(p_preg1)
par(mfrow = c(1, 1))
output_preg1=precis(p_preg1, depth=2 , digits=2)@output
plot(precis(p_preg1, pars=c("bp_rank","bm_rank","bp_pat","bm_pat","bp_next","bm_next","bp_group","bm_group"),depth=2))

plot(p_preg2)
par(mfrow = c(1, 1))
output_preg2=precis(p_preg2, depth=2 , digits=2)@output
plot(precis(p_preg2, pars=c("bp_rank","bm_rank","bp_pat","bm_pat","bp_no","bm_no","bp_next","bm_next","bp_group","bm_group"),depth=2))
...
#model comparisons
coefs5=coeftab(p_preg0,p_preg1,p_preg2,p_preg3,p_preg4)
coefsfinal=coeftab(p_preg0,p_preg2,p_preg4)
